...
from func_test import (
    determine_best_combination_for_all_intervals,
    determine_best_combination_for_one_by_one_interval,
    evaluate_strategies,
)
...
def calc_combinations(
    ...
    datasets_name = (
        ["validation", "test"]
        if specific_combination_params is None
        else specific_combination_params[4]
    )
            ...
            if calc_one_by_one_or_all == "ONE_BY_ONE":
                for date in train_starts:
                    ...
                    if nn_path_type[1] == "MAIN":
                        ...
                        df_10nn = join_external_dfs_from_one_interval(
                            nn_path_type[0], initializers_labels, date, dataset_name
                        )

                    pool.apply_async(
                        calc_metrics_for_combinations,
                        args=(
                            df_10nn,
                            combinations_list,
                            initializers_labels,
                ...
                df_nns_all = join_dfs_from_all_intervals(
                    nns_for_intervals, initializers_labels, train_starts, dataset_name
                )
...
def determine_best_combination_and_evaluate(
    combinations_list: list, train_starts: list, nn_type: str, combs_res_dir_name: str
) -> None:
    """ "
    ...
    for top_comb_share in [0.1, 0.05, 0.01]:
        best_one_by_one = determine_best_combination_for_one_by_one_interval(
            comb_list, train_starts, nn_type, top_comb_share, combs_res_dir_name
        )
        best_all_at_once = determine_best_combination_for_all_intervals(
            comb_list, nn_type, top_comb_share, combs_res_dir_name, best_one_by_one
        )
        evaluate_strategies(
            best_one_by_one,
            best_all_at_once,
    ...
    df_max_combinations_results.to_csv(
        os.path.join(combs_res_dir_name, "MAX_combinations_results.csv"), index=None
    )
...
def main() -> None:
    ...
    combinations_best_daily_prediction_dir_name = (
        predictions_dir_name_nn_type[0][0] + "_DAILY_BEST_COMBINATIONS/"
    )
    ...
    for strategy in [
        ["ALL_AT_ONCE", "ONE_BY_ONE_TESTED_ON_ALL_AT_ONCE"],
        ["ONE_BY_ONE", "ALL_AT_ONCE_TESTED_ON_ONE_BY_ONE"],
    ]:

        df = pd.read_csv(
            os.path.join(combinations_results_dir_name, "MAX_combinations_results.csv")
        )
        df = df.loc[
            (df["which_best"] == "BEST")
            & (df["which_strategy"].isin(strategy))
            & (df["which_approach"] == nn_type)
        ]

        for combination in df["combination"].unique():

            thold = df.loc[df["combination"] == combination, "threshold"].values[0]
            share = df.loc[df["combination"] == combination, "share"].values[0]
            method = df.loc[df["combination"] == combination, "which_strategy"].values[0]

            if method == "ONE_BY_ONE_TESTED_ON_ALL_AT_ONCE":
                method = "OBOT"
            elif method == "ALL_AT_ONCE":
                method = "AAOM"
            elif method == "ONE_BY_ONE":
                method = "OBOM"
            elif method == "ALL_AT_ONCE_TESTED_ON_ONE_BY_ONE":
                method = "AAOT"

            top_comb_share = df.loc[
                df["combination"] == combination, "which_top_comb_share"
            ].values[0]
            gain = df.loc[df["combination"] == combination, "which_gain"].values[0]

            combination = (
                combination.replace("[", "").replace("]", "").replace("'", "").replace(" ", "")
            )
