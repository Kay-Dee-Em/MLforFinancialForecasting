...
def join_external_dfs_from_one_interval(
    path: str, initializers_labels: list, train_start: str, dataset_name: str
) -> pd.DataFrame:
    """
    ...
    file_name = (
        "Prediction_" + dataset_name + "_" + initializers_labels[0] + "_" + train_start + ".csv"
    )
...
def join_dfs_from_all_intervals(
    nns_for_intervals: list, initializers_labels: list, train_starts: list, dataset_name: str
) -> pd.DataFrame:
    """
    ...
    for no_date in range(len(train_starts)):
        ...
        predictive_cols = (
            nns_for_intervals[no_date]
            .loc[:, nns_for_intervals[no_date].columns.str.startswith(col_name)]
            .columns.to_list()
        )
