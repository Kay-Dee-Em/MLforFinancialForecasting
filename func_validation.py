...
def calc_metrics_for_combinations(
    df: pd.DataFrame,
    combinations_list: list,
    initializers_labels: list,
    train_starts: list,
    dataset_name: str,
    nn_type: str,
    dir_name: str,
    calc_one_by_one_or_all: str,
    save_specific_combination: bool = False,
    specific_combination_params: tuple = None,
) -> None: 
    """ " 
    Calculate metrics (gain and accuracy) for all possible models' combinations (1023 combinations for 10-item list)
    Metrics are calculated based on predictions and given thresholds (uncertainty intervals 0.5 +- thold: 0, 0.005, 0.01, 0.025, 0.05)
    ...
    if save_specific_combination:
        ...
        calc_metrics_acc_thold(
            ...
            shares,
            comb_options,
            save_specific_combination,
            specific_combination_params,
        )

    else:

        if calc_one_by_one_or_all == "ONE_BY_ONE":
            print("START:", nn_type, dataset_name, train_starts[0], datetime.datetime.now())
        elif calc_one_by_one_or_all == "ALL_AT_ONCE":
            print("START:", nn_type, dataset_name, datetime.datetime.now())
        ...
        for combination in combinations_list[1:]:
            ...
            df_gain = df_gain[
                df_gain["share"]
                == min(df_gain["share"], key=lambda x: abs(x - df_gain["share"].median()))
            ]
            df_gain = df_gain[
                df_gain["threshold"]
                == min(df_gain["threshold"], key=lambda x: abs(x - df_gain["threshold"].median()))
            ]
            df_gain_list = np.array(df_gain[["threshold", "share", "accuracy", "gain"]])[0]

            df_acc = df_temp_max[df_temp_max["accuracy"] == df_temp_max["accuracy"].max()]
            df_acc = df_acc[
                df_acc["share"]
                == min(df_acc["share"], key=lambda x: abs(x - df_acc["share"].median()))
            ]
            df_acc = df_acc[
                df_acc["threshold"]
                == min(df_acc["threshold"], key=lambda x: abs(x - df_acc["threshold"].median()))
            ]
            df_acc_list = np.array(df_acc[["threshold", "share", "accuracy", "gain"]])[0]

            df_avg_gain = df_temp_max[df_temp_max["avg_gain"] == df_temp_max["avg_gain"].max()]
            df_avg_gain = df_avg_gain[
                df_avg_gain["share"]
                == min(df_avg_gain["share"], key=lambda x: abs(x - df_avg_gain["share"].median()))
            ]
            df_avg_gain = df_avg_gain[
                df_avg_gain["threshold"]
                == min(
                    df_avg_gain["threshold"],
                    key=lambda x: abs(x - df_avg_gain["threshold"].median()),
                )
            ]
            df_avg_gain_list = np.array(
                df_avg_gain[["threshold", "share", "avg_accuracy", "avg_gain"]]
            )[0]

            df_avg_acc = df_temp_max[
                df_temp_max["avg_accuracy"] == df_temp_max["avg_accuracy"].max()
            ]
            df_avg_acc = df_avg_acc[
                df_avg_acc["share"]
                == min(df_avg_acc["share"], key=lambda x: abs(x - df_avg_acc["share"].median()))
            ]
            df_avg_acc = df_avg_acc[
                df_avg_acc["threshold"]
                == min(
                    df_avg_acc["threshold"], key=lambda x: abs(x - df_avg_acc["threshold"].median())
                )
            ]
            df_avg_acc_list = np.array(
                df_avg_acc[["threshold", "share", "avg_accuracy", "avg_gain"]]
            )[0]
        ...
        df_comb_max_options = pd.DataFrame(
            comb_max_options,
            columns=[
                "combination",
                "MG_threshold",
...
def calc_metrics_acc_thold(
    df_combination: pd.DataFrame,
    combination: list,
    df_base_columns: list,
    initializers_labels: list,
    base_col_name: str,
    train_starts: list,
    thold: float,
    comb_accs_opt: list,
    comb_gains_opt: list,
    comb_avg_accs_opt: list,
    comb_avg_gains_opt: list,
    tholds: list,
    shares: list,
    comb_options: list,
    save_specific_combination: bool = False,
    ...
    # For predictions between uncertainty interval (0.5 +- threshold ('thold')) assign np.nan
    # Round predictions according to math's rule (columns 'Prediction_avg')
    ...
    df_comb_opt["Prediction_avg"] = np.nanmean(
        df_comb_opt[df_comb_opt.gt(-9999)][pred_col_names], axis=1
    )
    df_comb_opt.loc[
        (df_comb_opt["Prediction_avg"] >= (0.5 - thold))
        & (df_comb_opt["Prediction_avg"] <= (0.5 + thold)),
        "Prediction_avg",
    ] = np.nan
    ...
    for combination_el in combination:
        for train_start in train_starts:
            ...
            df_comb_opt.loc[
                (df_comb_opt[col_el_opt] >= (0.5 - thold))
                & (df_comb_opt[col_el_opt] <= (0.5 + thold)),
                col_el_opt,
            ] = np.nan
    ...
    decision_sum_cols = pd.Series(
        np.nansum(df_comb_opt[df_comb_opt.gt(-9999)][pred_col_names], axis=1)
    )
    decision_count_cols = pd.Series(
        df_comb_opt[df_comb_opt.gt(-9999)][pred_col_names].count(axis=1)
    )
    decision_count_nan_cols = (
        df_comb_opt[df_comb_opt.ge(-9999)][pred_col_names].isnull().sum(axis=1)
    )

    df_comb_smry = pd.DataFrame(
        [
            df_combination["DateTime"],
            df_comb_opt["Close"],
            df_comb_opt["Change"],
            df_comb_opt["Decision"],
            decision_sum_cols,
            decision_count_cols,
            decision_count_nan_cols,
        ],
    ...
    df_comb_smry.loc[df_comb_smry["How many NNs"] != 0, "Share Long Predictions"] = (
        df_comb_smry.loc[df_comb_smry["How many NNs"] != 0, "Long Predictions"]
        / df_comb_smry.loc[df_comb_smry["How many NNs"] != 0, "How many NNs"]
    )

    df_comb_smry["Share Long Predictions"] = (
        df_comb_smry["Long Predictions"] / df_comb_smry["How many NNs"]
    )
    ...
    if save_specific_combination:
        ...
        calc_metrics_acc_thold_and_share(
            df_comb_smry,
            combination,
            thold,
            round(share, 3),
...
def calc_metrics_acc_thold_and_share(
    ...
    df_smry.loc[
        (df_smry["NaNs Predictions"] / (df_smry["NaNs Predictions"] + df_smry["How many NNs"]))
        >= 0.5,
        "Final Prediction",
    ] = np.nan
    ...
    dec_accuracy = (
        np.nan
        if len(df_smry_wo_nans) == 0
        else (df_smry_wo_nans["Decision"] == df_smry_wo_nans["Final Prediction"]).sum()
        / len(df_smry_wo_nans["Decision"])
    )
    dec_gain = (
        (
            -abs(df_smry_wo_nans["Decision"] - df_smry_wo_nans["Final Prediction"])
            + (df_smry_wo_nans["Decision"] == df_smry_wo_nans["Final Prediction"])
        )
        * abs(df_smry_wo_nans["Change"])
    ).sum()
    dec_gain += df_smry_w_nans["Change"].sum()

    df_smry_wo_nans_avg = df_smry.loc[
        (df_smry["Final Prediction"].notnull()) & (df_smry["Average Prediction"].notnull())
    ].copy()
    df_smry_w_nans_avg = df_smry.loc[
        (df_smry["Final Prediction"].isnull()) | (df_smry["Average Prediction"].isnull())
    ].copy()
    ...
    dec_accuracy_by_avg = (
        np.nan
        if len(df_smry_wo_nans_avg) == 0
        else (df_smry_wo_nans_avg["Decision"] == df_smry_wo_nans_avg["Average Prediction"]).sum()
        / len(df_smry_wo_nans_avg["Decision"])
    )
    dec_gain_by_avg = (
        (
            -abs(df_smry_wo_nans_avg["Decision"] - df_smry_wo_nans_avg["Average Prediction"])
            + (df_smry_wo_nans_avg["Decision"] == df_smry_wo_nans_avg["Average Prediction"])
        )
        * abs(df_smry_wo_nans_avg["Change"])
    ).sum()
    dec_gain_by_avg += df_smry_w_nans_avg["Change"].sum()

    if save_specific_combination:

        df_smry["Final Prediction Change"] = (
            -abs(df_smry_wo_nans["Decision"] - df_smry_wo_nans["Final Prediction"])
            + (df_smry_wo_nans["Decision"] == df_smry_wo_nans["Final Prediction"])
        ) * abs(df_smry_wo_nans["Change"])
        df_smry.loc[
            df_smry["Final Prediction Change"].isnull(), "Final Prediction Change"
        ] = df_smry_w_nans["Change"]
        df_smry["Average Prediction Change"] = (
            -abs(df_smry_wo_nans_avg["Decision"] - df_smry_wo_nans_avg["Average Prediction"])
            + (df_smry_wo_nans_avg["Decision"] == df_smry_wo_nans_avg["Average Prediction"])
        ) * abs(df_smry_wo_nans_avg["Change"])
        df_smry.loc[
            df_smry["Average Prediction Change"].isnull(), "Average Prediction Change"
        ] = df_smry_w_nans_avg["Change"]

        if specific_combination_params[6] in ["OBOM", "AAOT"]:
            file_name = (
                specific_combination_params[5]
                + "_Predictions_for_"
                + specific_combination_params[6]
                + "_"
                + str(specific_combination_params[7])
                + "_"
                + specific_combination_params[8]
                + "_"
                + train_starts[0]
                + ".xlsx"
            )
        ...
        comb_accs_opt.append(dec_accuracy)
        comb_gains_opt.append(dec_gain)
        comb_avg_accs_opt.append(dec_accuracy_by_avg)
        comb_avg_gains_opt.append(dec_gain_by_avg)
        tholds.append(thold)
        shares.append(share)
