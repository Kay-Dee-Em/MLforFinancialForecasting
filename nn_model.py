...
from nn_model_auxiliary import ChannelAttention, SpatialAttention, StopOnPoint, ReshapeLayer
...
class NNModel:
    ...
    def __repr__(self) -> str:
        ...
        return f"\nModel(project_path = {self.project_path},\n \
    ...
    def create_train_and_evaluate_model(self) -> None:
        """
        ...
        """
        ...
        if not self.pass_model:
            ...
            GN, GU, HN, HU, LU = (
                GlorotNormal(seed=42),
                GlorotUniform(seed=42),
                HeNormal(seed=42),
                HeUniform(seed=42),
                LecunUniform(seed=42),
            )
            OR, RN, RU, TN, VS = (
                Orthogonal(seed=42),
                RandomNormal(seed=42),
                RandomUniform(seed=42),
                TruncatedNormal(seed=42),
                VarianceScaling(seed=42),
            )
    ...
    def gather_data_path_into_df(self) -> None:
        ...
        for number, subfolder in enumerate(["SHORT", "LONG"]):
            ...
            dates = [
                dt.split("/")[-1].split("\\")[-1].split(".")[0].replace("_", "-") for dt in images
            ]
            data_class = pd.DataFrame(
                {
                    "Image": images,
                    "Label": [str(number) + "_" + subfolder] * len(images),
                    "Date": dates,
                }
            )
        ...
        data = (
            data.merge(self.df_closing_prices, how="inner", on="Date")
            .sort_values(by="Date")
    ...
    def split_data_into_N_NNs(self) -> list:
        ...
        for i in range(self.NN_number):
            ...
            df_train_chunk = self.df_data_path.loc[
                self.df_data_path["DateTime"].between(train_start, train_end, inclusive="left")
            ].reset_index(drop=True)
            df_validation_chunk = self.df_data_path.loc[
                self.df_data_path["DateTime"].between(train_end, validation_end, inclusive="left")
            ].reset_index(drop=True)
            df_test_chunk = self.df_data_path.loc[
                self.df_data_path["DateTime"].between(validation_end, test_end, inclusive="left")
            ].reset_index(drop=True)
            ...
            print(
                i,
                "TRAIN:",
                str(df_train_chunk["DateTime"].dt.date[0]),
                str(df_train_chunk["DateTime"].dt.date[len(df_train_chunk) - 1]),
                "VALIDATION:",
                str(df_validation_chunk["DateTime"].dt.date[0]),
                str(df_validation_chunk["DateTime"].dt.date[len(df_validation_chunk) - 1]),
                "TEST:",
                str(df_test_chunk["DateTime"].dt.date[0]),
                str(df_test_chunk["DateTime"].dt.date[len(df_test_chunk) - 1]),
            )
    ...
    def train_model(self, NN_no, initializer) -> None:
        ...
        model_name = os.path.join(
            self.models_dir_name, "Model_" + initializer + start_NN_date + ".h5"
        )
        model_ckpoint = ModelCheckpoint(
            model_name,
            monitor="val_acc",
            mode="max",
            verbose=self.verbose,
        ...
        early_stoping = EarlyStopping(
            monitor="val_acc", min_delta=0.001, patience=self.patience, verbose=self.verbose
        )

        model_fit = self.models_list[0].fit(
            train_data,
            epochs=self.epochs,
    ...
    def evaluate_model(
        ...
        if not self.pass_model:
            if initializer == "VS":
                df_validation.to_csv(
                    os.path.join(self.predictions_dir_name, col_val_name + ".csv"), index=False
                )
        else:
            df_validation.to_csv(
                os.path.join(self.predictions_dir_name, col_val_name + ".csv"), index=False
        ...
        if not self.pass_model:
            if initializer == "VS":
                df_test.to_csv(
                    os.path.join(self.predictions_dir_name, col_test_name + ".csv"), index=False
                )
        else:
            df_test.to_csv(
                os.path.join(self.predictions_dir_name, col_test_name + ".csv"), index=False
