...
class ImageGenerator:
    ...
    def preprocess_data(self) -> None:
        ...
        df = pd.read_csv(
            os.path.join(self.project_path, self.data_name),
            names=col_names,
            header=None,
            sep=self.separator,
        )
        ...
        df = (
            df[df["DateTime"].dt.weekday < 5]
            .set_index("DateTime")
            .between_time(self.h_start, self.h_end)
            .reset_index()
        )
        ...
        df = (
            df[~df["DateTime"].isin(Calendar().holidays(start=data_start, end=data_end))]
            .fillna(method="ffill")
        ...
        df_closing_prices = self.df.loc[
            self.df["DateTime"].dt.date.astype(str).isin(self.days_end)
        ].reset_index(drop=True)
        df_closing_prices = (
            df_closing_prices.groupby(df_closing_prices["DateTime"].dt.date)
            .apply(lambda x: x.iloc[-1])
            .reset_index(drop=True)
        )
        df_closing_prices["Change"] = -df_closing_prices["Close"].diff(periods=-1)
        df_closing_prices["Decision"] = df_closing_prices["Change"].apply(
    ...
    def create_images(self) -> None:
        ...
        for i in range(len(self.days_end)):
            pool.apply_async(
                self.calc_freqs_and_set_decision,
                args=(i, self.days_start[i], self.days_end[i], self.days_next_after_end[i]),
                callback=self.__get_result,
            )
        pool.close()
        pool.join()
        ...
        logger.success(
            f"GENERATING IMAGES FINISHED SUCCESSFULLY\nTotal Days: {total_days}\nTotal Images Created: {imgs_created}\
    ...
    def calc_freqs_and_set_decision(
        self, i: int, day_start: str, day_end: str, day_next_after_end: str
    ) -> tuple:
        """
        ...
        df_interval = self.df.loc[
            (self.df["DateTime"] >= day_start) & (self.df["DateTime"] < day_next_after_end)
        ]
        ...
        for freq in self.freqs:
            ...
            group_dt = (
                df_interval.groupby(pd.Grouper(key="DateTime", freq=freq, offset=offset_hour))
                .mean()
                .reset_index()
            )
        ...
        future_value = self.df[self.df["DateTime"].dt.date.astype(str) == day_next_after_end][
            "Close"
        ].iloc[-1]
    ...
    def generate_gaf(self, one_day_data: list, decision: str) -> None:
        ...
        self.join_gafs_into_heatmap(
            images=gafs,
            image_name="{0}".format(one_day_data[0].replace("-", "_")),
    ...
    def join_gafs_into_heatmap(self, images: list, image_name: str, destination: str) -> None:
        ...
        fig = plt.figure(
            figsize=[
                img * self.image_matrix[0] * self.image_matrix[1] for img in self.image_matrix
            ]
        )
