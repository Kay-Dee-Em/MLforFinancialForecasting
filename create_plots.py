...
def strategy_visualization(df: pd.DataFrame, date_long_start: str = '2021-12-03', date_long_end_next_day: str = '2022-01-01',
                                             ...
                                             date_short_pred_start: str = '2021-11-09', date_short_pred_end_next_day: str = '2021-11-11',
                                             title_prefix: str = '', plots_dir_name: str = 'PLOTS') -> None:
    ...
    fig_names = [title_prefix + 'LONG_' + date_long_pred_start.replace('-', '_') + '_line', title_prefix + 'SHORT_' + date_short_pred_start.replace('-', '_') + '_line',
                 title_prefix + 'LONG_' + date_long_pred_start.replace('-', '_') + '_polar', title_prefix + 'SHORT_' + date_short_pred_start.replace('-', '_') + '_polar']
...
def cross_validation_visualization(df: pd.DataFrame, train_start: str = '2001-04-02', len_train: int = 13*12, len_validation: int = 3.75*12, len_test: int = 2*12,
                                   interval_in_month: int = 3, NN_number: int = 9, title_prefix: str = '', plots_dir_name: str = 'PLOTS') -> None:
    ...
    fig.savefig(os.path.join(plots_dir_name, title_prefix + 'cross_validation.png'), bbox_inches='tight', pad_inches=0, transparent=True)
    plt.close()
...
def cross_validation_test_visualization(df: pd.DataFrame, train_start: str = '2001-04-02', test_start: str = '2018-01-01', test_end_next_day: str = '2022-01-01',
                                        len_train: int = 13*12, len_validation: int = 3.75*12, len_test: int = 2*12, interval_in_month: int = 3, 
                                        NN_number: int = 9, title_prefix: str = '', plots_dir_name: str = 'PLOTS') -> None:
    ...
    for i in range(NN_number):
        ...
        if i == NN_number-1:
            ...
            axs[0].vlines(x=x_test[len(x_test)-1], ymin=10, ymax=y_vals[len(y_vals)-1]-10, colors='#06474D', ls='-.', lw=2, label='koniec')
    ...
    fig.savefig(os.path.join(plots_dir_name, title_prefix + 'cross_validation_test.png'), bbox_inches='tight', pad_inches=0, transparent=True)
    plt.close()



def test_data_changes_histogram(df: pd.DataFrame, test_start: str = '2018-01-01', test_end_next_day: str = '2022-01-01', title_prefix: str = '', plots_dir_name: str = 'PLOTS') -> None:

    plt.figure(figsize=(10,7))

    df_x = df.loc[df['DateTime'].between(test_start, test_end_next_day, inclusive='left')]
    plt.hist(df_x.loc[df_x['Change'] > 0]['Change'], color='#7FC866', edgecolor='black', linewidth=1.2, bins=10, label='LONG') 
    plt.hist(df_x.loc[df_x['Change'] < 0]['Change'], color='#EE613F', edgecolor='black', linewidth=1.2, bins=10, label='SHORT') 
    ...
    plt.savefig(os.path.join(plots_dir_name, title_prefix + 'hist_test_data.png'), bbox_inches='tight', pad_inches=0, transparent=True)
    plt.close()



def predictions_histogram(data_dir_name: str, predictions_files: list, title_prefix: str = '', plots_dir_name: str = 'PLOTS') -> None:
    ...
    for pred_file in predictions_files:
        ...
        plt.hist(df_true, color='#7FC866', edgecolor='black', linewidth=1.2, bins=10, label='PRAWDA') 
        plt.hist(df_false, color='#EE613F', edgecolor='black', linewidth=1.2, bins=10, label='FAŁSZ', alpha = 0.75) 
        ...
        plt.savefig(os.path.join(plots_dir_name, title_name_prefix + 'hist_prediction.png'), bbox_inches='tight', pad_inches=0, transparent=True)
        plt.close()



def prediction_change_visualization(data_dir_name: str, predictions_files: list, label_for_dataset: str = 'OIH: BUY & HOLD', title_prefix: str = '', plots_dir_name: str = 'PLOTS') -> None:
    ...
    plt.grid(True)
    plt.savefig(os.path.join(plots_dir_name, title_prefix + 'prediction.png'), bbox_inches='tight', pad_inches=0, transparent=True)
    plt.close()



def prediction_visualization(data_dir_name: str, predictions_files: list, label_for_dataset: str = 'OIH: BUY & HOLD', title_prefix: str = '', plots_dir_name: str = 'PLOTS') -> None:
    ...
    for file_no, pred_file in enumerate(predictions_files):
        ...
        plt.plot(x_true, y_true + (file_no+2)*6+2, color=colors[file_no], linewidth=0.1, alpha=0.4, marker='.')

        # HOLD GAIN
        df = df_main.copy()
        df.loc[df[decision].isnull(), 'Decision_HOLD'] = 'HOLD_' + (df.loc[df[decision].isnull(), 'Decision'] + (file_no+2)*6).astype(int).astype(str)
        df.loc[df[decision].isnull(), decision] = df.loc[df[decision].isnull(), 'Decision'] 
        df['Prediction'] = df[decision]
        x_null = df['DateTime'].apply(lambda x: x[:10])[::step]
        y_null = df['Prediction'][::step]

        x_true = np.ma.masked_where('HOLD_' + (y_null + (file_no+2)*6).astype(int).astype(str) != df['Decision_HOLD'][::step], x_null)
        y_true = np.ma.masked_where('HOLD_' + (y_null + (file_no+2)*6).astype(int).astype(str) != df['Decision_HOLD'][::step], y_null)

        plt.plot(x_true, y_true + (file_no+2)*6, color='black', linewidth=3, linestyle='-', alpha=1)
        plt.plot(x_true, y_true + (file_no+2)*6, color='black', linewidth=0.1, alpha=0.4, marker='.')
    ...
    plt.grid(True)
    plt.savefig(os.path.join(plots_dir_name, title_prefix + 'prediction_raw.png'), bbox_inches='tight', pad_inches=0, transparent=True)
    plt.close()



def predictions_heatmap_overall(df: pd.DataFrame, approach_type: str = 'MAIN', title_prefix: str = '', heatmaps_dir_name: str = 'HEATMAPS') -> None:
    ...
    for strategy in ['ONE_BY_ONE_TESTED_ON_ALL_AT_ONCE', 'ALL_AT_ONCE']:
        for top_share in [0.1, 0.05, 0.01]:
            ...
            ax = sns.heatmap(heatmap, cmap='RdYlGn', linewidths=1, square=True, cbar_kws={'shrink': .9, 'label': 'Zysk'}, annot=True, fmt='.0f', annot_kws={'fontsize':8}, norm=norm)
            ...
            plt.savefig(os.path.join(heatmaps_dir_name, title_prefix + file_name), bbox_inches='tight', pad_inches=0, transparent=True, dpi=200)
            plt.close()



def predictions_heatmap_by_interval(df: pd.DataFrame, train_starts: list, approach_type: str = 'MAIN', title_prefix: str = '', heatmaps_dir_name: str = 'HEATMAPS') -> None:
...
def main() -> None:
    ...
    predictions_files =  [file for file in files_in_dir if ('Predictions_for' in file) and not(any(date in file for date in train_starts))]

    prediction_change_visualization(predictions_daily_dir_name, predictions_files, plots_dir_name = plots_dir_name)
    prediction_visualization(predictions_daily_dir_name, predictions_files, plots_dir_name = plots_dir_name)
    predictions_histogram(predictions_daily_dir_name, predictions_files, plots_dir_name = plots_dir_name)
    ...
    predictions_heatmap_by_interval(df_predictions, train_starts = train_starts, heatmaps_dir_name = heatmaps_dir_name)
