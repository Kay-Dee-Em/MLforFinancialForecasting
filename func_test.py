...
def determine_best_combination_for_one_by_one_interval(
    comb_list_len: int,
    train_starts: list,
    nn_type: str,
    top_comb_share: float,
    combs_res_dir_name: str,
) -> tuple:
    """ "
    ...
    for date in train_starts:
        ...
        val_result_max = (
            df_result.loc[
                df_result["MG_gain"].nlargest(int(np.floor(comb_list_len * top_comb_share))).index
            ]["combination"]
            if val_max > val_avg_max
            else df_result.loc[
                df_result["AVG_MG_gain"]
                .nlargest(int(np.floor(comb_list_len * top_comb_share)))
                .index
            ]["combination"]
        )
    ...
    top_combinations = df_top_combinations.loc[
        df_top_combinations.values == max(df_top_combinations)
    ].index.to_list()
    ...
    for date in train_starts:
        ...
        file_name = nn_type + "_Max_validation_" + date + ".csv"
        df_result = pd.read_csv(os.path.join(combs_res_dir_name, file_name))
        top_comb_temp = df_result.loc[df_result["combination"].isin(top_combinations)].sort_values(
            by=which_gain, ascending=False
        )[["combination", which_gain]]
    ...
    top_of_the_top_combinations = top_combinations_mean.loc[
        top_combinations_mean[which_gain] == top_gain
    ].index.to_list()
    top_combination = max(top_of_the_top_combinations, key=len)

    # Determine which threshold was the most common for chosen top combination
    tholds, shares = [], []
    for date in train_starts:
        ...
        thold_counts = df_result.loc[:, which_gain[:-4] + "threshold"].value_counts()
    ...
    shares_counts_all = df_shares.groupby(df_shares.index).sum() * (
        1 - abs(0.5 - df_shares.groupby(df_shares.index).sum().index)
    )
...
def determine_best_combination_for_all_intervals(
    comb_list_len: int,
    nn_type: str,
    top_comb_share: float,
    combs_res_dir_name: str,
    best_one_by_one: tuple,
) -> tuple:
    """ "
    Auxiliary function for determine_best_combination_and_evaluate (main description in determine_best_combination_and_evaluate)
    Determine the best combinations for main and side model
    ...
    top_combinations = (
        ...
        if val_max > val_avg_max
            ...
            df_result["AVG_MG_gain"].nlargest(int(np.floor(comb_list_len * top_comb_share))).index
        ]["combination"]
    )
    ...
    which_gain = (
        "MG_gain" if val_max > val_avg_max else "AVG_MG_gain"
    )  # add change if mean is the same
    top_gain = (
        df_result.loc[df_result["combination"].isin(top_combinations)]
        .sort_values(by=which_gain, ascending=False)[which_gain]
        .iloc[0]
    )
    top_of_the_top_combinations = df_result.loc[
        df_result[which_gain] == top_gain, "combination"
    ].to_list()
    top_combination = max(top_of_the_top_combinations, key=len)

    thold_test = df_result.loc[:, which_gain[:-4] + "threshold"].value_counts().index[0]
    share_counts = df_result.loc[:, which_gain[:-4] + "share"].value_counts()
...
def evaluate_strategies(
    best_one_by_one: tuple,
    best_all_at_once: tuple,
    top_comb_share: float,
    train_starts: list,
    nn_type: str,
    combs_res_dir_name: str,
    max_combinations_results: list,
) -> None:
    """ "
    Auxiliary function for determine_best_combination_and_evaluate (main description in determine_best_combination_and_evaluate)
    ...
    :param best_one_by_one: tuple, the best combination, threshold, share of positive predictions and gain type (main or as average) for one by one strategy
    :param best_all_at_once: tuple, the best combination, threshold, share of positive predictions and gain type (main or as average) for all at once intervals strategy
    :param top_comb_share: float, share of largest gain
    :param train_starts: list, list od start dates in format 'YYYY-MM-DD'
    :param nn_type: str, name of the approach type i.e. 'MAIN' (main model) or 'SIDE' (side model)
    :param combs_res_dir_name: str, directory where files are located
    ...
    # Having the best combination, approach to gain (standard gain or average gain), threshold and share
    ...
    for date in train_starts:
        ...
        gain = df_result.loc[
            (
                (df_result["combination"] == top_combination)
                & (df_result["threshold"] == thold_test)
                & (df_result["share"] == share_test)
            ),
            which_test_gain,
        ].iloc[0]
        accuracy = df_result.loc[
            (
                (df_result["combination"] == top_combination)
                & (df_result["threshold"] == thold_test)
                & (df_result["share"] == share_test)
        ...
        df_result.loc[
            df_result["combination"] == top_combination, "which_accuracy"
        ] = which_test_accuracy
        df_result.loc[
            df_result["combination"] == top_combination, "which_top_comb_share"
        ] = top_comb_share
        df_result.loc[
            (df_result["combination"] == top_combination)
            & ((df_result["threshold"] == thold_test) & (df_result["share"] == share_test)),
            "which_best",
        ] = "BEST"
    ...
    df_result.loc[
        df_result["combination"] == top_combination, "which_strategy"
    ] = "ONE_BY_ONE_TESTED_ON_ALL_AT_ONCE"
    df_result.loc[df_result["combination"] == top_combination, "which_approach"] = nn_type
    df_result.loc[df_result["combination"] == top_combination, "which_gain"] = which_test_gain
    df_result.loc[
        df_result["combination"] == top_combination, "which_accuracy"
    ...
    for date in train_starts:
        ...
        df_result.loc[
            df_result["combination"] == top_combination, "which_strategy"
        ] = "ALL_AT_ONCE_TESTED_ON_ONE_BY_ONE"
        df_result.loc[df_result["combination"] == top_combination, "which_approach"] = nn_type
        df_result.loc[df_result["combination"] == top_combination, "which_gain"] = which_test_gain
        df_result.loc[
            df_result["combination"] == top_combination, "which_accuracy"
